      ...
      {isOpen && options && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900/50 rounded-3xl p-8 max-w-sm w-full shadow-2xl text-center transform scale-100 transition-all flex flex-col items-center animate-in fade-in zoom-in duration-200">
            ...
            <p className="text-gray-600 dark:text-slate-300 mb-8 leading-relaxed">{options.message}</p>
            
            {options.type === 'confirm' ? (
              <div className="flex gap-3 w-full">
                <button 
                  onClick={handleCancel}
                  className="flex-1 py-3 rounded-xl font-bold text-gray-600 dark:text-slate-300 bg-gray-100 hover:bg-gray-200 transition-colors"
                >
                  Batal
                </button>
                <button 
                  onClick={handleConfirm}
                  className="flex-1 py-3 rounded-xl font-bold text-white bg-indigo-600 hover:bg-indigo-700 shadow-md dark:shadow-none shadow-indigo-200 transition-colors"
                >
                  Ya, Lanjutkan
                </button>
              </div>
              ...
              <button
                ...
                className={`w-full py-3.5 rounded-xl font-bold text-white transition-colors ${
                  options.type === 'success' 
                    ? 'bg-green-600 hover:bg-green-700 shadow-green-200 shadow-md dark:shadow-none' 
                    : options.type === 'error'
                    ? 'bg-red-600 hover:bg-red-700 shadow-red-200 shadow-md dark:shadow-none'
                    : 'bg-indigo-600 hover:bg-indigo-700 shadow-indigo-200 shadow-md dark:shadow-none'
                }`}
              >
                OK, Mengerti
              </button>
